    ...
    <div class="content">
        ...
        <?php if ($slideNotes !== '' || ($isFirst && $topNotes !== '') || ($isLast && $tailNotes !== '')): ?>
            ...
            <div class="speaker-notes-buttons">
                ...
                <?php if ($slideNotes !== ''): ?>
                    <button
                        ...
                        type="button"
                        data-notes-target="speaker-notes-<?= $slideNumber ?>"
                    >
                        Speaker Notes
                    </button>
                <?php endif; ?>
                ...
                <?php if ($isFirst && $topNotes !== ''): ?>
                    <button
                        class="speaker-notes-button"
                        type="button"
                        data-notes-target="speaker-notes-top"
                    >
                        Opening Notes
                    </button>
                <?php endif; ?>
                ...
                <?php if ($isLast && $tailNotes !== ''): ?>
                    <button
                        class="speaker-notes-button"
                        type="button"
                        data-notes-target="speaker-notes-tail"
                    >
                        Closing Notes
                    </button>
                <?php endif; ?>
            ...
            <?php if ($slideNotes !== ''): ?>
                <div
                    class="speaker-notes"
                    id="speaker-notes-<?= $slideNumber ?>"
                    hidden
                >
                    <div class="speaker-notes-panel" role="dialog" aria-modal="true" aria-labelledby="speaker-notes-title-<?= $slideNumber ?>">
                        <div class="speaker-notes-header">
                            <h2 id="speaker-notes-title-<?= $slideNumber ?>">Speaker Notes</h2>
                            <button class="speaker-notes-close" type="button" data-notes-close aria-label="Close speaker notes">&times;</button>
                        </div>
            ...
            <?php if ($isFirst && $topNotes !== ''): ?>
                <div
                    class="speaker-notes"
                    id="speaker-notes-top"
                    hidden
                >
                    <div class="speaker-notes-panel" role="dialog" aria-modal="true" aria-labelledby="speaker-notes-title-top">
                        <div class="speaker-notes-header">
                            <h2 id="speaker-notes-title-top">Opening Notes</h2>
                            <button class="speaker-notes-close" type="button" data-notes-close aria-label="Close opening notes">&times;</button>
                        </div>
                        <div class="speaker-notes-body">
            ...
            <?php if ($isLast && $tailNotes !== ''): ?>
                <div
                    class="speaker-notes"
                    id="speaker-notes-tail"
                    hidden
                >
                    <div class="speaker-notes-panel" role="dialog" aria-modal="true" aria-labelledby="speaker-notes-title-tail">
                        <div class="speaker-notes-header">
                            <h2 id="speaker-notes-title-tail">Closing Notes</h2>
                            <button class="speaker-notes-close" type="button" data-notes-close aria-label="Close closing notes">&times;</button>
                        </div>
                        <div class="speaker-notes-body">
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('[data-notes-target]').forEach(function (button) {
        button.addEventListener('click', function () {
            const notes = document.getElementById(button.dataset.notesTarget);
            if (notes) notes.hidden = false;
        });
    });

    document.querySelectorAll('[data-notes-close]').forEach(function (button) {
        button.addEventListener('click', function () {
            const notes = button.closest('.speaker-notes');
            if (notes) notes.hidden = true;
        });
    });

    document.querySelectorAll('.speaker-notes').forEach(function (notes) {
        notes.addEventListener('click', function (event) {
            if (event.target === notes) notes.hidden = true;
        });

        const panel = notes.querySelector('.speaker-notes-panel');
        const header = notes.querySelector('.speaker-notes-header');
        ...
        if (panel && header) {
            ...
            header.addEventListener('pointerdown', function (event) {
                if (event.target.closest('[data-notes-close]')) return;
                dragging = true;
    ...
    document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape') {
            document.querySelectorAll('.speaker-notes:not([hidden])').forEach(function (notes) {
                notes.hidden = true;
            });
        }
    });
